...
'* Version:     5.2
'* Patched by Ger Hobbelt ([i_a]):   2006-2010
'*
...
'********************************************************************
    ...
    Const CONST_CurrentBuild           = "5.2.2[i_a]"
    ...
    'FSO is used in several functions, so lets set it globally.
    Set fso = WScript.CreateObject("Scripting.FileSystemObject")
    If blnErrorOccurred(" occurred in getting FileSystemObject. (Msg#102)") Then WScript.Quit

    strTakeOwnToolPath = mkFullPath4Tool(fso, "takeown.exe")
    ' Call PrintMsg("takeown = " & strTakeOwnToolPath)
    ...
    Do
        ...
        Case CONST_PROCEED
            ...
            ' [i_a] turn on quiet mode once we start running through the items
            If qq_Used And Not blnQuiet Then
                If Not q_Used Then Call PrintMsg("")
    ...
    If qq_Used And blnQuiet And Not q_used Then
        Call PrintMsg("")
...
Sub DoTheWorkOnEverythingUnderDirectory(byval ThisPath)
    ...
    Do
        ...
        Else
            If debug_on then
                Call PrintMsg("DoTheWorkOnEverythingUnderDirectory: DirScan: """ & ThisPath & """")
            End If

            Set f = fso.GetFolder(ThisPath)
            If blnErrorOccurred(" occurred in getting FileSystemObject.GetFolder (Msg#203)") Then Exit Do

            If debug_on And False then
                Call PrintMsg("DoTheWorkOnEverythingUnderDirectory: Scanned: " & f.DateCreated & ", " & f.Attributes)
                If f.Attributes AND 2 Then
                    Call PrintMsg("Hidden folder.")
                End If
                If f.Attributes AND 4 Then
                    Call PrintMsg("System folder.")
                End If
                If f.Attributes AND 16 Then
                    Call PrintMsg("Folder.")
                End If
                If f.Attributes AND 32 Then
                    Call PrintMsg("Archive bit set.")
                End If
                If f.Attributes AND 2048 Then
                    Call PrintMsg("Compressed folder.")
                End If
                Call PrintMsg("Date created: " & f.DateCreated)
                Call PrintMsg("Date last accessed: " & f.DateLastAccessed)
                Call PrintMsg("Date last modified: " & f.DateLastModified)
                Call PrintMsg("Drive: " & f.Drive)
                Call PrintMsg("Is root folder: " & f.IsRootFolder)
                Call PrintMsg("Name: " & f.Name)
                Call PrintMsg("Parent folder: " & f.ParentFolder)
                Call PrintMsg("Path: " & f.Path)
                Call PrintMsg("Short name: " & f.ShortName)
                Call PrintMsg("Short path: " & f.ShortPath)
                Call PrintMsg("Size: " & f.Size)
                Call PrintMsg("Type: " & f.Type)
            End If

            Set fc = f.Files
            If debug_on then
                Call PrintMsg("DoTheWorkOnEverythingUnderDirectory: Files")
            End If
            For Each f1 in fc
                If debug_on Then
                    Call PrintMsg("DoTheWorkOnEverythingUnderDirectory: File: """ & f1.Path & """")
                End If
            ...
            Set fc = f.SubFolders
            If debug_on then
                Call PrintMsg("DoTheWorkOnEverythingUnderDirectory: SubDirectories")
            End If
            For Each f1 in fc
                If debug_on then
                    Call PrintMsg("DoTheWorkOnEverythingUnderDirectory: SubDirectory: """ & f1.Path & """")
                End If
...
Sub DisplayThisACL(byval strPath)
    ...
    'Put statements in loop to be able to drop out and clear variables
    Do
        ' [i_a] retry the objService.Get() after a takeown fix IF an error occurred the first time!
        RetryOFSquery = 2
        Do While (RetryOFSquery > 0)
            Set objFileSecSetting = Nothing
            If Not IsObject(objService) Then
                If debug_on then
                    Call PrintMsg("SetACLForObject: fatal error *** objService is not an object! ***")
                End If
            End If

            Set objFileSecSetting = objService.Get("Win32_LogicalFileSecuritySetting.Path=""" & Replace(strPath,"\","\\") & """")
            If blnErrorOccurredDontIgnore(" occurred setting Win32_LogicalFileSecuritySetting object. (Msg#401)") Then
                ' [i_a] on error, force generic ownership before anything else!
                '       NOTE: no use to run takeown again when already did!
                If RetryOFSquery > 1 And blnForceTakeOwnership Then  'Change Ownership
                    If debug_on then Call PrintMsg("DisplayThisACL: Changing Ownership to generic Administrators to reset ownership to something that should work")
                    
                    Dim WshShell, RunResult
                    
                    Set WshShell = WScript.CreateObject("WScript.Shell")
                    RunResult = WshShell.Run(strTakeOwnToolPath & " """ & strPath & """", 7, True)
                    Set WshShell = Nothing

                    If debug_on then Call PrintMsg("DisplayThisACL: takeown """ & strPath & """ returned: " & RunResult)
                    
                    RetryOFSquery = RetryOFSquery - 1
                Else
                    ' unresolvable error: quit retrying and signal failure
                    RetryOFSquery = -1
                End If
                ...
                If debug_on then
                    ...
                    If RetryOFSquery < 0 Then
                        Call PrintMsg("DisplayThisACL:           Failed Irrecoverably")
                    Else
                        If RetryOFSquery > 0 Then
                            Call PrintMsg("DisplayThisACL:           Retrying (retry count remaining: " & RetryOFSquery & ")")
                        Else
                            Call PrintMsg("DisplayThisACL:           Retry limit reached.")
                        End If
                    End If
                End If
            Else
...
Sub SetACLForObject(byval strPath, byval IsItAFolder)
    ...
    Do
        ...
        ' [i_a] always force generic ownership before anything else when both /FO and /O have been specified!
        If blnForceTakeOwnership Then
            Call PrintMsg("Forced Ownership Change to generic Administrators")

            Set WshShell = WScript.CreateObject("WScript.Shell")
            RunResult = WshShell.Run(strTakeOwnToolPath & " """ & strPath & """", 7, True)
            Set WshShell = Nothing

            If debug_on then Call PrintMsg("SetACLForObject: takeown returned: " & RunResult)
        End If
        ...
        Do While (RetryOFSquery > 0)
            ...
            Set objFileSecSetting = objService.Get("Win32_LogicalFileSecuritySetting.Path=""" & Replace(strPath,"\","\\") & """")
            If blnErrorOccurredDontIgnore(" occurred setting Win32_LogicalFileSecuritySetting object. (Msg#501)") Then
                ' [i_a] on error, force generic ownership before anything else!
                '       NOTE: no use to run takeown again when already did!
                '       And don't care whether o_Used (/O) was used when /FO was used as well 
                If RetryOFSquery > 1 And (o_Used Or blnForceTakeOwnership) Then  'Change Ownership
                    If debug_on then Call PrintMsg("SetACLForObject: Changing Ownership to generic Administrators to reset ownership to something that should work")
                    If o_Var <> "" then
                        If Not ObjTrustee_o_var_User Is Nothing Then
                            ...
                            If debug_on then Call PrintMsg("SetACLForObject: takeown """ & strPath & """ returned: " & RunResult)
                        End If
                    End If
                    RetryOFSquery = RetryOFSquery - 1
                Else
                ...
                If debug_on then
                    ...
                    If RetryOFSquery < 0 Then
                        Call PrintMsg("SetACLForObject:           Failed Irrecoverably")
                    Else
                        If RetryOFSquery > 0 Then
                            Call PrintMsg("SetACLForObject:           Retrying (retry count remaining: " & RetryOFSquery & ")")
                        Else
                            Call PrintMsg("SetACLForObject:           Retry limit reached.")
                        End If
                    End If
                End If
            Else
        ...
        If (r_Used and i_var<>3) or i_var=2 then    'We must get the Parent DACL in case one of those users are Revoked, Also if we want to Copy the Inheritance
            DoStoreACL = True ' [i_a] change detected: store ACL!
            If debug_on then
                Call PrintMsg("SetACLForObject: Change detected: r_used = " & r_Used & ", i_var = " & i_var)
            End If
        ...
        If i_Var_Copy_Temp then
            DoStoreACL = True ' [i_a] change detected: store ACL!
            If debug_on then
                Call PrintMsg("SetACLForObject: Change detected: i_Var_Copy_Temp = " & i_Var_Copy_Temp)
            End If
                        ...
                        For t = LBound(p_var_User) to UBound(p_var_User)
                            If p_Var_User(t) <> "" then
                                If TrusteesMatch(ObjTrustee_p_var_User(t), objTempTrustee) then
                                    DoStoreACL = True ' [i_a] change detected: store ACL!
                                    If debug_on then
                                        Call PrintMsg("SetACLForObject: Change detected: p_used = " & p_used & ", p_var_User = " & p_var_User)
                                    End If
                        ...
                        For t = LBound(r_var_User) to UBound(r_var_User)
                            If r_Var_User(t) <> "" then
                                If TrusteesMatch(ObjTrustee_r_var_User(t), objTempTrustee) then
                                    DoStoreACL = True ' [i_a] change detected: store ACL!
                                    If debug_on then
                                        Call PrintMsg("SetACLForObject: Change detected: r_used = " & r_used & ", r_var_User = " & r_var_User)
                                    End If
        ...
        If g_Used then  'Grant rights for these users
            ...
            DoStoreACL = True ' [i_a] change detected: store ACL!
            If debug_on then
                Call PrintMsg("SetACLForObject: Change detected: g_used = " & g_used)
            End If
        ...
        If p_Used then  'Grant rights for these users (Replace rights)
            ...
            DoStoreACL = True ' [i_a] change detected: store ACL!
            If debug_on then
                Call PrintMsg("SetACLForObject: Change detected: p_used = " & p_used)
            End If
        ...
        If d_Used then  'Deny rights for these users
            ...
            DoStoreACL = True ' [i_a] change detected: store ACL!
            If debug_on then
                Call PrintMsg("SetACLForObject: Change detected: d_used = " & d_used)
            End If
        ...
        If UBound(ImpDenyDaclObj) > 0 then      '0 member is always blank
            For i = (LBound(ImpDenyDaclObj) + 1) to UBound(ImpDenyDaclObj)
                DoStoreACL = True ' [i_a] change detected: store ACL!
                If debug_on then
                    Call PrintMsg("SetACLForObject: Change detected: UBound(ImpDenyDaclObj) = " & UBound(ImpDenyDaclObj))
                End If
        ...
        If UBound(ImpAllowDaclObj) > 0 then
            For i = (LBound(ImpAllowDaclObj) + 1) to UBound(ImpAllowDaclObj)
                DoStoreACL = True ' [i_a] change detected: store ACL!
                If debug_on then
                    Call PrintMsg("SetACLForObject: Change detected: UBound(ImpAllowDaclObj) = " & UBound(ImpAllowDaclObj))
                End If
            ...
            If o_Var <> "" then
                If Not ObjTrustee_o_var_User Is Nothing then
                    ...
                    Else
                        DoStoreACL = True ' [i_a] change detected: store ACL!
                        If debug_on then
                            Call PrintMsg("SetACLForObject: Change detected: o_Used = " & o_Used & ", ObjTrustee_o_var_User = " & TrusteesDisplay(ObjTrustee_o_var_User))
                        End If
        ...
        If (DoStoreACL) Then ' [i_a] change detected: store ACL!
            If debug_on then
                Call PrintMsg("SetACLForObject: Change detected: save ACL")
            End If
            ...
            Set objmethod = Nothing
        Else
            Call PrintMsg("No change.  (Msg#545)")
        End If
...
Function SECBitMask(byval strsec)
    ...
    If debug_on then Call PrintMsg("SECBitMask: Enter: " & strsec)
    ...
    Do
        ...
        For i = LBound(OurPermsSelected) to UBound(OurPermsSelected)
            ' [i_a] bugfix: do NOT add permission bits as duplicate entries in the input will
            ' thus causing the bit to be added multiple times ==> corrupted permissions bitfield.
            'SECBitMask = SECBitMask + OurPermsSelected(i)
            '
            ' OR is bitwise, see: http://blogs.msdn.com/ericlippert/archive/2004/07/15/184431.aspx
            SECBitMask = SECBitMask Or OurPermsSelected(i)
        Next
        Exit Do
    Loop

    If debug_on then Call PrintMsg("SECBitMask: Return = 0x" & Hex(SECBitMask))
...
Function StringAceFlag(ByVal numAceFlags, ByVal numControlFlags, ByVal FlagToCheck, ByVal ReturnShort, ByRef ReturnAceFlags)
    ...
    If debug_on then Call PrintMsg("StringAceFlag: Enter(numAceFlags=" & numAceFlags & ", numControlFlags=" & numControlFlags & ", FlagToCheck=" & FlagToCheck & ", ReturnShort=" & ReturnShort & ", ReturnAceFlags=" & ReturnAceFlags)
...
Function DoesPathNameExist(byVal strFilePath)
    ...
    If debug_on then Call PrintMsg("DoesPathNameExist: Enter: " & strFilePath)
...
Sub PrintArguments()
    ...
    End If
    If fo_Used then
        Call PrintMsg(vbtab & "/FO (Force [Administrators] Ownership)")
    End If
    If i_Used then
    ...
    End If
    If qq_Used then
        Call PrintMsg(vbtab & "/QQ (Semi-quiet mode)")
    End If
    If c_Used then
        Call PrintMsg(vbtab & "/C (Ignore Errors)")
    End If
    If debug_Used then
                ...
                If boolNeedsColon then
                    If colonplace > 0 then
                        ...
                        If strPerm <> "" and strSpec = "" then
                            ' If IsPermGUI(strPerm) then   -- [i_a] makes '/G user:DCBA987654321' act different from the equiv. '/G user:F' :-(
                            If semicolonplace <= 0 then
                                ' If no ';' has been specified, we say these are simple rights.
                                strSpec = "*GUI*"
                            End If
                        End If
...
Sub ShowUsage()
    ...
    Call PrintMsg("                [/O user] [/FO] [/I ENABLE/COPY/REMOVE] [/N")
    Call PrintMsg("                [/L filename] [/Q] [/QQ] [/C] [/DEBUG]")
    Call PrintMsg("")
    ...
    Call PrintMsg("                                      (If any user has SID# then globally all")
    ...
    Call PrintMsg("                                    R  Read")
    Call PrintMsg("                                    W  Write")
    Call PrintMsg("                             Advanced... (Assumes ""/SPEC G"")")
    ...
    Call PrintMsg("")
    Call PrintMsg("   /P user:GUI         Replace security permissions similar to standard choices")
    ...
    Call PrintMsg("")
    Call PrintMsg("   /D user:GUI         Deny security permissions similar to standard choices.")
    ...
    Call PrintMsg("")
    Call PrintMsg("   /FO                 Always force an Ownership change using the takeown tool.")
    Call PrintMsg("")
    ...
    Call PrintMsg("                                   into Effective ACL's")
    ...
    Call PrintMsg("                       If filename is omitted the default name of XCACLS.log will")
    ...
    Call PrintMsg("")
    Call PrintMsg("   /QQ                 Turn on Semi-Quiet mode: show a '.' dot for every processed item.")
    Call PrintMsg("")
    Call PrintMsg("   /C                  Ignore errors, its off by default.")
    Call PrintMsg("")
    ...
    Call PrintMsg("                       Sub/Function Enter and Exit as well as other important")
...
Sub OpenOutputFile()
    ...
    Do
        ...
        If Not objFileSystem.FileExists(strAbsoluteFile) Then
            ...
            MyFile.Close
        End If
        ' Create file for appending, create if not exist, write ASCII format
        set objOutputFile = objFileSystem.OpenTextFile(strAbsoluteFile, ForAppending, TRUE)
    ...
    If Not blnQuiet then
        If debug_on then
            Wscript.Echo "OpenOutputFile: Exit"
        End If
    End If
...
Function blnErrorOccurred (byval strIn)
    ...
    blnErrorOccurred = blnErrorOccurredDontIgnore(strIn)
    ' [i_a] ignore errors and try to continue as best we can
    If blnIgnoreErrors Then
        blnErrorOccurred = False
    End If
End Function

Function blnErrorOccurredDontIgnore (byval strIn)
    ...
    If Err.Number Then
        ...
        If TempDescript <> "" Then
            If TempNum = -2147217406 Then ' Error 0x80041002 : Object Not Found
                ' The system delivers the error string without the word 'Object' which was rather confusing :-(
                TempDescript = "Object " & TempDescript
            End If
            Call PrintMsg( "Error description: " & TempDescript)
        End If

        If qq_Used And blnQuiet And qq_MsgBuffer <> "" Then
            Wscript.Echo qq_MsgBuffer
            qq_MsgBuffer = ""
        End If

        ' [i_a] cannot ignore these errors and try to continue as best we can
        blnErrorOccurredDontIgnore = True
    Else
        blnErrorOccurredDontIgnore = False
    End If
End Function
    ...
    If Not blnQuiet Then
        ...
        If l_Used Then
            If strOutputFile <> "" Then
                ...
                End If
                Err.Number = TempNum
                Err.Description = TempDescript
            End If
        End If
    Else
        If qq_Used Then
            If qq_MsgBuffer <> "" Then
                qq_MsgBuffer = qq_MsgBuffer & vbCRLF
            End If
            qq_MsgBuffer = qq_MsgBuffer & strMessage
        End If
    End If
End Sub
...
'
' Wshell.Run does not resolve PATH, so any executable run that way must get an ABSOLUTE
' path before it will work out okay.
'
' As we make sure the executables (takeown.exe in our case) resides in the same directory
' as the xcacls script, we're good to go when we prepend the full path to the script
' to the executable(s) as well.
'
Function mkFullPath4Tool(byval fso, byval exeName)
    Dim path
    
    path = Wscript.ScriptFullName
    path = fso.GetParentFolderName(path)
    
    mkFullPath4Tool = fso.BuildPath(path, exeName)
End Function
